...
class ImagePromptWorker:
    ...
    @staticmethod
    def _conditionings(references: List[ReferenceBinding]) -> List[ConditioningSpec]:
        conditionings: List[ConditioningSpec] = []
        for reference in references:
            if reference.use == ReferenceUse.CHARACTER_IDENTITY:
            ...
            elif reference.use in {
                ReferenceUse.FIRST_FRAME,
                ReferenceUse.LAST_FRAME,
            }:
                conditionings.append(
                    ConditioningSpec(
                        conditioning_type=ConditioningType.KEYFRAME,
                        reference_id=reference.reference_id,
                        strength=1.0,
                        required=reference.required,
                    )
                )
            elif reference.use == ReferenceUse.VIDEO:
                conditionings.append(
                    ConditioningSpec(
                        conditioning_type=ConditioningType.VIDEO,
                        reference_id=reference.reference_id,
                        strength=1.0,
                        required=reference.required,
                    )
                )
            elif reference.use == ReferenceUse.STYLE:
                conditionings.append(
                    ConditioningSpec(
                        conditioning_type=ConditioningType.STYLE,
                        reference_id=reference.reference_id,
                        strength=1.0,
                        required=reference.required,
                    )
    ...
    def execute(
        ...
        for sc in scenes:
            ...
            visual_plan = VisualPromptPlan(
                ...
                conditionings=self._conditionings(references),
            )
